...
exports.createUser = (req, res) => {
	...
	if(typeof firstName !== "string" || typeof lastName !== "string") return res.status(400).json({error: "wrong type!!!!"})
	if (!isAlphanumeric(firstName) || !isAlphanumeric(lastName))return res.status(400).json({ error: "That name is not allowed !" })
	db.run(
